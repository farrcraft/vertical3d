...
SLToken::SLToken(Kind kind, unsigned int line, unsigned int column) :
    kind_(kind),
    line_(line),
    column_(column) {
}

SLToken::SLToken(Kind kind, const std::string & text, unsigned int line, unsigned int column) :
    kind_(kind),
    text_(text),
    line_(line),
    column_(column) {
...
SLToken::SLToken(float value, unsigned int line, unsigned int column) :
    kind_(Kind::NUMBER),
    value_(value),
    line_(line),
    column_(column) {
...
SLToken::Kind SLToken::kind() const {
    return kind_;
}

const std::string & SLToken::text() const {
    return text_;
}

float SLToken::value() const {
    return value_;
}

unsigned int SLToken::line() const {
    return line_;
}

unsigned int SLToken::column() const {
    return column_;
}

SLLexer::SLLexer(std::istream & stream) : stream_(stream) {
}
...
SLToken SLLexer::fail(const std::string & message, unsigned int line, unsigned int column) {
    if (error_.empty()) {
        error_ = message + position(line, column);
    }
    return SLToken(SLToken::Kind::END, line, column);
}

SLToken SLLexer::scanString(unsigned int line, unsigned int column) {
    ...
    return SLToken(SLToken::Kind::STRING, text, line, column);
}

SLToken SLLexer::scanNumber(unsigned int line, unsigned int column) {
    ...
    return SLToken(std::strtof(text.c_str(), nullptr), line, column);
}

SLToken SLLexer::scanName(unsigned int line, unsigned int column) {
    std::string text;
    while (alpha(look()) || digit(look())) {
        text += static_cast<char>(get());
    }
    const SLToken::Kind kind = keyword(text) ? SLToken::Kind::KEYWORD : SLToken::Kind::IDENTIFIER;
    return SLToken(kind, text, line, column);
}

SLToken SLLexer::scanOperator(unsigned int line, unsigned int column) {
    const int first = get();
    ...
    if (assigns || doubled) {
        text += static_cast<char>(get());
        return SLToken(SLToken::Kind::OPERATOR, text, line, column);
    }
    if (first == '&' || first == '|') {
...
SLToken SLLexer::scan() {
    if (!error_.empty()) {
        return SLToken(SLToken::Kind::END, line_, column_);
    }
    if (!skipSpace()) {
        return SLToken(SLToken::Kind::END, line_, column_);
    }
    ...
    if (c == '.') {
        ...
        return SLToken(SLToken::Kind::OPERATOR, ".", line, column);
    }
    if (alpha(c)) {
    ...
    if (punctuation(c)) {
        get();
        return SLToken(SLToken::Kind::PUNCTUATION, std::string(1, static_cast<char>(c)), line, column);
    }
    if (c == '+' || c == '-' || c == '*' || c == '/' || c == '^' || c == '=' || c == '!' ||
        c == '<' || c == '>' || c == '&' || c == '|' || c == '?' || c == ':') {
...
const std::string & SLLexer::error() const {
    return error_;
}
