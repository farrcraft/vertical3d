...
class SLValue final {
 public:
     ...
     **/
    void reset(SLType type, SLStorage storage, unsigned int batch);

    SLType type() const;
    SLStorage storage() const;
    /**
     ...
     **/
    void assign(const SLValue & other, unsigned int point);

 private:
    SLType type_ = SLType::FLOAT;
    SLStorage storage_ = SLStorage::UNIFORM;
