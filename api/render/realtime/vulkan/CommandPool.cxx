...
namespace v3d::render::realtime::vulkan {
    ...
    /**
     **/
    CommandPool::CommandPool(const boost::shared_ptr<Device>& device, uint32_t family) :
        device_(device),
        ...
        if (result != VK_SUCCESS) {
            ...
            msg << "Unable to create a vulkan command pool - " << resultString(result);
    ...
    std::vector<VkCommandBuffer> CommandPool::allocate(uint32_t count) const {
        ...
        if (result != VK_SUCCESS) {
            std::stringstream msg;
            msg << "Unable to allocate vulkan command buffers - " << resultString(result);
            throw std::runtime_error(msg.str());
        }
