...
namespace v3d::render::realtime::vulkan {
    ...
    /**
     **/
    Buffer::Buffer(const boost::shared_ptr<Device>& device, VkBufferUsageFlags usage, VkDeviceSize bytes) :
        device_(device),
        usage_(usage),
    ...
    void Buffer::create(VkDeviceSize bytes) {
        ...
        if (result != VK_SUCCESS) {
            ...
            msg << "Unable to create a vulkan buffer - " << resultString(result);
        ...
        if (result != VK_SUCCESS) {
            ...
            buffer_ = VK_NULL_HANDLE;
            std::stringstream msg;
            msg << "Unable to allocate memory for a vulkan buffer - " << resultString(result);
            throw std::runtime_error(msg.str());
        }
        ...
        if (result != VK_SUCCESS) {
            destroy();
            std::stringstream msg;
            msg << "Unable to bind memory to a vulkan buffer - " << resultString(result);
            throw std::runtime_error(msg.str());
        }
        ...
        if (result != VK_SUCCESS) {
            destroy();
            std::stringstream msg;
            msg << "Unable to map a vulkan buffer - " << resultString(result);
            throw std::runtime_error(msg.str());
        }
