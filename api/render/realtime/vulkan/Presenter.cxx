...
/**
 **/
Presenter::Presenter(const boost::shared_ptr<v3d::log::Logger>& logger, const boost::shared_ptr<Device>& device,
...
void Presenter::createSync() {
    ...
    for (uint32_t index = 0; index < framesInFlight_; index++) {
        ...
        if (result != VK_SUCCESS) {
            ...
            msg << "Unable to create a vulkan semaphore - " << resultString(result);
        ...
        if (result != VK_SUCCESS) {
            std::stringstream msg;
            msg << "Unable to create a vulkan fence - " << resultString(result);
            throw std::runtime_error(msg.str());
        }
...
Presenter::Status Presenter::acquire(Acquisition* acquisition) {
    ...
    if (result != VK_SUCCESS && result != VK_SUBOPTIMAL_KHR) {
        std::stringstream msg;
        msg << "Unable to acquire a vulkan swapchain image - " << resultString(result);
        throw std::runtime_error(msg.str());
    }
    ...
    if (result != VK_SUCCESS) {
        std::stringstream msg;
        msg << "Unable to reset a vulkan frame fence - " << resultString(result);
        throw std::runtime_error(msg.str());
    }
    ...
    if (result != VK_SUCCESS) {
        std::stringstream msg;
        msg << "Unable to reset a vulkan command buffer - " << resultString(result);
        throw std::runtime_error(msg.str());
    }
    ...
    if (result != VK_SUCCESS) {
        std::stringstream msg;
        msg << "Unable to begin a vulkan command buffer - " << resultString(result);
        throw std::runtime_error(msg.str());
    }
...
Presenter::Status Presenter::present(const Acquisition& acquisition) {
    ...
    if (result != VK_SUCCESS) {
        std::stringstream msg;
        msg << "Unable to end a vulkan command buffer - " << resultString(result);
        throw std::runtime_error(msg.str());
    }
    ...
    if (result != VK_SUCCESS) {
        std::stringstream msg;
        msg << "Unable to submit a vulkan frame - " << resultString(result);
        throw std::runtime_error(msg.str());
    }
    ...
    if (result != VK_SUCCESS) {
        std::stringstream msg;
        msg << "Unable to present a vulkan swapchain image - " << resultString(result);
        throw std::runtime_error(msg.str());
    }
