...
class TextureFactory final {
 public:
     ...
     **/
    explicit TextureFactory(const boost::shared_ptr<Device>& device);
     ...
     **/
    Texture create(const unsigned char* pixels, uint32_t width, uint32_t height, uint32_t channels) const;

    /**
     * @param image the image to upload, whose bpp decides the channel count
     **/
    Texture create(const boost::shared_ptr<v3d::image::Image>& image) const;
