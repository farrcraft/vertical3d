...
files and no use at all of the include root [ADR-0027](../adr/0027-the-api-is-consumed-as-source.md)
put there, so every move done before the conversion rewrites `../` counts in libraries and apps
that had nothing to do with it. Afterwards a move is a `git mv`, a `CMakeLists.txt` edit and a
namespace line. The seven moves are independent of each other, with two exceptions: `ui::Style`
joins `ui/style/` before `api/ui` regroups, and the `api/render/offline` split waits for
[OfflineRenderingPhase3](OfflineRenderingPhase3.md) to close, since that phase is rewriting most
of the files it renames.
