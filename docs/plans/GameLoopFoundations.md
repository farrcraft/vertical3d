...
Drafted 2026-09-05, staged into this tree 2026-09-06, **open**. Three things the game loop owes
every app that subclasses it, and one that is nearly free once the first lands. None of them is new capability — each is a gap that every
...
| ADR | Decision |
|---|---|
| **0032** — to write | The loop simulates at a fixed step and renders at a variable one — **step 1 writes it** |
...
Move [`vertical3d/src/view/CameraProfiles.h`/`.cxx`](../../vertical3d/src/view/CameraProfiles.h) to
`api/config/CameraProfiles.h`/`.cpp`, namespace `v3d::editor` → `v3d::config`. Rename the
...
- Fixed rate as a constant or configurable (step 1). Constant is the recommendation.
- Should the accumulator be its own small type in `api/engine`, or private state on `Engine`? A
  separate type is testable without standing up an engine, which argues for it, but it is a public
  header either way.
- Do tetris and voxel eventually move onto `simulate()`? They are not wrong today, so there is no
  forcing reason — but leaving two apps on a different timing model than the other two is the kind
  of split that is invisible until someone copies the wrong one.
