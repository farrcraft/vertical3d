...
Drafted 2026-09-08 from a survey of `api/`, and **closed** the same day. Eleven steps: three
...
| 4 `render/offline` | landed | 10 `type` | landed |
...
in the parent namespace. [UiConsolidation](UiConsolidation.md) step 5 gave the three
...
- **`3dtypes.h`** declares `floor_log2` and `npot` at **global
...
files. That reasoning assumed concurrent branches; both workstreams are sequential commits on
one branch, so there was nothing to conflict with. The real cost was that phase 3's own plan
named files by their old names, and it is updated with this step.

```
offline/             FrameBuffer
offline/rib/         Lexer Declarations Parameters Reader Handler      v3d::render::offline::rib
offline/sl/          Types Lexer Syntax Parser Builtins Compiler Emitter   …::offline::sl
offline/sl/runtime/  Value Program Machine Renderer                    …::offline::sl::runtime
```

**The `sl`/`runtime` boundary was not real until this step made it so.** `SLValue.h` and
`SLProgram.h` both included `SLSyntax.h` — the AST — for the three type enums, so a split alone
would have left the runtime reaching the parser and the directory would have said otherwise.
The enums moved into `sl/Types.h`, which already existed and is about exactly those types, and
`Syntax.h` now includes `Types.h` rather than the reverse. `runtime/` depends on `sl/Types.h`
and on nothing else above it, and `sl/Emitter` is the one file in the compiler half that names
a runtime type — it writes a `runtime::Program`, which is the seam.

**The prefixes came off as a namespace went on**, 55 symbols in all: `RIBLexer` and `SLLexer`
are `rib::Lexer` and `sl::Lexer`, which is the pair the old names were working around.
`RIBDeclaration`/`RIBDeclarations`, `SLType`/`SLTypes` and `SLShader`/`SLShaderType` are prefixes
of each other, and a word-boundary strip of the whole prefix handled them where a longest-first
list would also have been needed.

**The scoping mattered more than the renaming.** `RIBHandler` names three classes: the api
interface, and moya's and talyn's implementations of it. Outside the library only the
fully-qualified `v3d::render::offline::RIBHandler` was rewritten, so
[`moya::RIBHandler`](../../../moya/libmoya/RIBHandler.h) keeps its name and derives from
`offline::rib::Handler`. After the collisions in steps 7 and 10, this step replaced nothing by
bare name outside the directory it was moving.
...
`Style.{h,cpp}` moves into [`api/ui/style/`](../../../api/ui/style/) and
...
[UiConsolidation](UiConsolidation.md) step 5 on the three `Style`s — so a rename by
