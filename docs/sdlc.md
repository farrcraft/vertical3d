...
[`plans/ApiOrganisation.md`](plans/ApiOrganisation.md) **is open at step 4 alone**. It settled
the include convention every file in the tree had disagreed with `examples/starter` about
([ADR-0048](adr/0048-an-api-header-is-named-from-the-repository-root.md)) and split five of the
six `api/` directories that held more than one thing; `api/render/offline` waits for the phase
rewriting it.
