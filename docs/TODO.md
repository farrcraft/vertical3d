...
[] the seam that feeds the keyboard router is unbuilt: nothing in this tree constructs a `ui::Keys` or calls `Engine::focusFirst()`, and the editor builds a `ui::Cursor` with no `Keys`, no `Measure` and no `Clipboard` - it has no text box to need any of them. Every case for the routing is in `api/ui/tests`, so it is proven and the wiring an app has to write is not
