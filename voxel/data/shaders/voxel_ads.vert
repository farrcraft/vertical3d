...
uniform MaterialInfo materials[16];
...
void main()
{
	...
	int materialId = int(voxelInfo.y) - 1;
	...
	vec3 ambient = Light.La * materials[materialId].Ka;
	float sDotN = max(dot(s, tnorm), 0.0);
	vec3 diffuse = Light.Ld * materials[materialId].Kd * sDotN;
	vec3 spec = vec3(0.0);

	if (sDotN > 0.0)
	{
		spec = Light.Ls * materials[materialId].Ks * pow(max(dot(r, v), 0.0), materials[materialId].Shininess);
	}
